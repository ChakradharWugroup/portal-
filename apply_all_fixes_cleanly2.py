...
# 1. Imports
if "import { useMsal" not in content:
    content = content.replace(
        "import React, { useState, useEffect, useRef } from 'react';",
        "import { useMsal } from '@azure/msal-react';\nimport React, { useState, useEffect, useRef } from 'react';"
    )
if "Bot, Megaphone" not in content:
    content = content.replace(
...
# 2. useMsal hook
if "const { instance, accounts } = useMsal();" not in content:
    content = content.replace(
        "export default function App() {",
        "export default function App() {\n  const { instance, accounts } = useMsal();"
    )

# 3. Add handleMicrosoftLogin (loginRedirect)
if "await instance.loginRedirect" not in content:
    content = content.replace(
...
# 4. Auto-Login useEffect & Logout fix
if "instance.logoutRedirect" not in content:
    # First, find setUserProfile
    content = content.replace(
        """  const handleLogout = () => {
    setIsLoggedIn(false);
  };""",
    ...
    # Insert useEffect after userProfile state
    content = content.replace(
...
# 5. Iframes
if "nextchat': 'NextChat" not in content:
    content = content.replace(
...
if "src=\"http://localhost:3001\"" not in content:
    iframes_html = """
...
# 6. Login Form Strip (Only match exactly the manual fields)
pattern = re.compile(
...
with open(filepath, 'w', encoding='utf-8') as f:
    f.write(content)
print("Complete safe restore and modification applied!")
